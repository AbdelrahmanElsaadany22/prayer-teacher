...
export class FriendsController {
  ...
  @Post('/:receiverId')
  sendRequest(@Req() req, @Param('receiverId') receiverId: string) {
  ...
  rejectRequest(@Req() req, @Param('requestId') requestId: string) {
    return this.friendReqService.rejectRequest(req.user.id, requestId);
  }

  @Delete('cancel/:requestId')
  cancelRequest(@Req() req, @Param('requestId') requestId: string) {
    return this.friendReqService.cancelRequest(req.user.id, requestId);
  }

  @Delete('unfriend/:friendId')
  removeFriend(@Req() req, @Param('friendId') friendId: string) {
    return this.friendReqService.removeFriend(req.user.id, friendId);
  }
}
